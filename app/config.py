...
def _env_or_config(key: str, default=None, prefix: str = "OPENMANUS_"):
    """Get config value from env var or return default.
    Env var format: OPENMANUS_<SECTION>_<KEY> (uppercase)
    """
...
class Config:
    ...
    def _get_config_path() -> Path:
        ...
        raise FileNotFoundError(
            ...
            "Copy config/config.example.toml to config/config.toml and edit it with your API keys.\n"
            "Or set environment variables like OPENMANUS_LLM_API_KEY=your_key"
        )
    ...
    def _apply_env_overrides(self, raw_config: dict) -> dict:
        ...
        Format: OPENMANUS_LLM_API_KEY overrides [llm] api_key
        """
        for section_key in raw_config:
            section = raw_config[section_key]
            if isinstance(section, dict):
                for field_key in section:
                    env_val = _env_or_config(f"{section_key}_{field_key}")
                    if env_val is not None:
                        section[field_key] = env_val

        # Also check for standalone env vars for critical settings
        api_key = _env_or_config("LLM_API_KEY")
        if api_key and "llm" in raw_config:
            raw_config["llm"]["api_key"] = api_key
